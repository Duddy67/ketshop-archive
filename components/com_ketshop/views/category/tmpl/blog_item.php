...
<div class="product-item">
  ...
  <?php if($this->item->attribute_group) : //Check for product options. ?>
    ...
    <a href="<?php echo JRoute::_(KetshopHelperRoute::getProductRoute($this->item->slug, $this->item->catid, $this->item->language)); ?>">
  ...
  <?php if($params->get('show_product_page_link')) :
	  if($params->get('access-view')) :
	    $link = JRoute::_(KetshopHelperRoute::getProductRoute($this->item->slug, $this->item->catid, $this->item->language));
	  else : //Redirect the user to the login page.
	    $menu = JFactory::getApplication()->getMenu();
	    $active = $menu->getActive();
	    $itemId = $active->id;
	    $link = new JUri(JRoute::_('index.php?option=com_users&view=login&Itemid='.$itemId, false));
	    $link->setVar('return', base64_encode(JRoute::_(KetshopHelperRoute::getProductRoute($this->item->slug, $this->item->catid, $this->item->language), false)));
	  endif; ?>
